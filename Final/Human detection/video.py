...
import imutils
import argparse
import json #For comm with server
...
class people:
    ...
    def detect_people(frame):
        ...
        '''
        print(rectangle) 
        cv2.line(frame,(int(200),0),(int(200),int(300)),(0,0,255),2) # (400,300 Webcam)    
        
        for (x, y, w, h) in rectangle:
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
            if abs(x+0.5*w-200)<50:      
                print("Stay")                
                if (y>50): 
                    ch='w'
                    print("Forward")
            ...
            #cv2.line(frame,(int(x+0.5*w),y),(int(x+0.5*w),int(y+h)),(0,0,255),2) TO draw central rect line
...
if __name__ == '__main__':
    ...
    #camera = cv2.VideoCapture(1) #for external web cam
    ...
    while True:
        ...
        frame_orginal = imutils.resize(
            frame, width=min(400, frame.shape[1]))
